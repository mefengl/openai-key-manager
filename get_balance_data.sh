...
function format_data() {
	...
	for DATA in "${RESULTS[@]}"; do
		...
		echo "Access until date: $ACCESS_UNTIL_DATE"

		TABLE1_HEADERS=("Account Name" "Object" "Has Payment Method" "Canceled" "Access Until"
			"Plan Title" "Plan ID")
		TABLE1_DATA=("${VALUES[0]}" "${VALUES[1]}" "${VALUES[2]}" "${VALUES[3]}" "$ACCESS_UNTIL_DATE" "${VALUES[13]}" "${VALUES[14]}")
		TABLE1_COLORS=("$CYAN" "$YELLOW" "$BLUE" "$PURPLE" "$GREEN" "$YELLOW" "$BLUE")
		TABLE2_HEADERS=("Soft Limit (USD)" "Hard Limit (USD)" "System Hard Limit (USD)")
		TABLE2_DATA=("${VALUES[10]}" "${VALUES[11]}" "${VALUES[12]}")
		TABLE2_COLORS=("$CYAN" "$YELLOW" "$BLUE")

		TABLE3_HEADERS=("Soft Limit" "Hard Limit" "System Hard Limit")
		TABLE3_DATA=("${VALUES[7]}" "${VALUES[8]}" "${VALUES[9]}")
		TABLE3_COLORS=("$CYAN" "$YELLOW" "$BLUE")

		TABLE4_HEADERS=("Canceled At" "Delinquent" "PO Number" "Billing Email" "Tax IDs" "Billing Address" "Business Address")
		TABLE4_DATA=("${VALUES[4]}" "${VALUES[5]}" "${VALUES[15]}" "${VALUES[16]}" "${VALUES[17]}" "${VALUES[18]}" "${VALUES[19]}")
		TABLE4_COLORS=("$CYAN" "$YELLOW" "$BLUE" "$PURPLE" "$GREEN" "$YELLOW" "$BLUE")
		...
		echo -e "${GREEN}${DIVIDER}${RESET}"
		render_table TABLE4_DATA[@] TABLE4_HEADERS[@] TABLE4_COLORS[@]
		echo -e "${GREEN}${DIVIDER}${RESET}"
	done
}
